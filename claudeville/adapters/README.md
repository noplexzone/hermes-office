...
Normalized session ids use independently base64url-encoded components (`hermes.<encoded-profile>.<encoded-session>`), named profiles become stable `agentName` and `profile` values, and parent ids are normalized within the same discovered profile. Jarvis, Light, and L use stable profile-specific Sol, Luna, and Terra character silhouettes while their real model labels remain unchanged. Unknown or malformed tool names become `custom_tool`. Hermes support requires Node.js 22.13 or newer for no-flag `node:sqlite`.

Project assignment prefers authoritative `git_repo_root` and `cwd`. When those fields are absent, the adapter may derive a lower-case synthetic path such as `/hermes-projects/radarr` only from explicit project/repository/branch wording in bounded `origin_json` display fields (`auto_thread_initial_name`, `chat_topic`, or `chat_name`); child sessions inherit a resolved parent project. Raw origin JSON, titles, unrelated display text, chat/user identifiers, and source strings never cross the adapter boundary.
